...
func TestIngressController(t *testing.T) {
	...
	RegisterFakeOnDelete(ingresses, &fedClient.Fake, fedIngressWatch, ingressObjectGetter)
	...
	*/
	...
	// test deleting ingress
	t.Log("Deleting Federated Ingress")
	fedIngress.ObjectMeta.Finalizers = append(fedIngress.ObjectMeta.Finalizers, deletionhelper.FinalizerDeleteFromUnderlyingClusters)
	fedIngress.DeletionTimestamp = &metav1.Time{Time: time.Now()}
	fedIngressWatch.Modify(&fedIngress)
	fedIngressWatch.Delete(&fedIngress)

	err := ingressController.delete(&fedIngress)
	if err != nil {
		t.Errorf("Failed to delete ingress %v", err)
	} else {
		t.Logf("Successfully deleted ingress %v", fedIngress)
	}
...
func ingressObjectGetter(name, namespace string) runtime.Object {
	ingress := new(extensionsv1beta1.Ingress)
